...
function Container({
    ...
    return (
        <main
            className={` ${nopadding ? "px-0" : "px-4"} ${
                margin && "mt-10"
            } flex min-h-full flex-col   items-center xl:px-0  ${
                debug && "bg-red-100 outline"
            }`}
        >
            <div
                className={`flex w-full h-[calc(100vh-62px)]  overflow-y-auto  max-w-7xl flex-col ${margin ? "pt-6" : null} `}
            >
                {children}
            </div>
